...
export default function QuizSessionPage() {
  ...
  const wrap = { minHeight: '100vh', fontFamily: "'Segoe UI', sans-serif", display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: S.bg }

  if (stage === 'loading') return <div style={wrap}><p style={{ color: S.muted }}>Loading quiz…</p></div>

  if (stage === 'ended') return (
    <div style={wrap}>
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontSize: 40, marginBottom: 12 }}>👋</div>
        <p style={{ color: S.text, fontSize: 18 }}>{error || 'This quiz session has ended.'}</p>
      </div>
    </div>
  )
  ...
  if (stage === 'identify') return (
    <div style={wrap}>
      <form onSubmit={identify} style={{ background: '#fff', padding: 32, borderRadius: 12, maxWidth: 380, width: '100%', border: `1px solid ${S.border}` }}>
        ...
        />
        {error && <div style={{ color: S.red, fontSize: 13, marginBottom: 12 }}>{error}</div>}
        <button type="submit" style={{ width: '100%', padding: 14, background: S.purple, color: '#fff', border: 'none', borderRadius: 8, fontSize: 16, fontWeight: 700, cursor: 'pointer' }}>
          Start Quiz
        </button>
      </form>
    </div>
  ...
  if (stage === 'quiz') {
    ...
    return (
      <div style={{ minHeight: '100vh', fontFamily: "'Segoe UI', sans-serif", background: S.bg, padding: 24 }}>
        <div style={{ maxWidth: 560, margin: '0 auto' }}>
          <div style={{ fontSize: 13, color: S.muted, marginBottom: 8 }}>Question {current + 1} of {quiz.questions.length}</div>
          <div style={{ background: '#fff', border: `1px solid ${S.border}`, borderRadius: 12, padding: 24, marginBottom: 20 }}>
            <p style={{ color: S.text, fontSize: 18, marginBottom: 20 }}>{q.question}</p>
            {q.options.map((opt, i) => (
              <button
                key={i}
                ...
                style={{
                  ...
                  border: answers[current] === i ? `2px solid ${S.purple}` : `1px solid ${S.border}`,
                  color: S.text,
                }}
              >
                {opt}
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={() => setCurrent((c) => Math.max(0, c - 1))} disabled={current === 0}
              style={{ padding: '12px 20px', background: '#fff', border: `1px solid ${S.border}`, borderRadius: 8, cursor: current === 0 ? 'default' : 'pointer', opacity: current === 0 ? 0.4 : 1 }}>
              Back
            </button>
            {isLast ? (
              <button onClick={submit} disabled={answers[current] === undefined}
                style={{ flex: 1, padding: '12px 20px', background: S.green, color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, cursor: 'pointer' }}>
                Submit Quiz
              </button>
            ) : (
              <button onClick={() => setCurrent((c) => c + 1)} disabled={answers[current] === undefined}
                style={{ flex: 1, padding: '12px 20px', background: S.text, color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, cursor: 'pointer' }}>
                Next
              </button>
            )}
          </div>
  ...
  if (stage === 'submitting') return <div style={wrap}><p style={{ color: S.muted }}>Submitting…</p></div>
  ...
  if (stage === 'done') return (
    <div style={{ minHeight: '100vh', fontFamily: "'Segoe UI', sans-serif", background: S.text, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <button onClick={retake} style={{ padding: '10px 22px', background: S.purple, color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, cursor: 'pointer', fontSize: 14 }}>
          Try Again
        </button>
